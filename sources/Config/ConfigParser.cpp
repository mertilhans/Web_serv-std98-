...
//Private members
// Validation -- bu dosya formatina OZEL kabul kriterleri (asagida neden
// Utils katmaninda OLMADIGI aciklaniyor, bkz. ConfigParser.hpp).
...
ListenConfig	ConfigParser::_parseListenValue(const std::string& raw)
{
	...
	uint32_t			ip;
	if (!NetUtils::resolveIPv4(hostPart, ip))
		throw std::runtime_error("Config: 'listen' unresolvable host");
	// RFC 6890: 224.0.0.0/4 (multicast, Class D) ve 240.0.0.0/4 (reserved,
	// Class E, 255.255.255.255 limited broadcast dahil) hicbir host'a
	// atanamaz -- bind() OS seviyesinde bunlara istisnayla izin veriyor
	// (RTN_MULTICAST/RTN_BROADCAST sahiplik kontrolunden muaf), bu yuzden
	// gecerli gorunup pratikte hicbir istemcinin erisemeyecegi bir soket
	// acilirdi.
	if ((ntohl(ip) >> 24) >= 224)
		throw std::runtime_error("Config: 'listen' multicast/reserved (Class D/E) address not allowed");
...
void	ConfigParser::_parseLocationDirective(LocationConfig& location)
{
	...
	else if (key == "return" || key == "redirect")
	{
		...
		if (!StringUtils::isNumeric(codeTok))
