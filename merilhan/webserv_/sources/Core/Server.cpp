...
// Graceful shutdown drain suresi -- cgi.hpp'deki CGI_TIMEOUT_SECONDS
// (5s) degerinden KASITLI OLARAK BUYUK: draining sirasinda hala calisan
// mesru bir CGI'nin kendi 5 saniyelik hakkini tam kullanabilmesi icin.
...
Server::~Server()
{
    ...
    // calismaya devam ederdi (normalde 5 saniyelik CGI timeout korumasi
...
void Server::processClient(int fd)
{
	...
	// sokmuyoruz. CGI bitince handleCgiPipeEvent cevabi gonderecek;
	// pipelining ile gelmis bir sonraki istek varsa o da (keep-alive'da
	// oldugu gibi) response yazildiktan sonra islenir. mCgiSessions outFd
	// ile anahtarli oldugu icin clientFd'ye gore tarama yapiyoruz.
	for (std::map<int, CgiSession>::iterator it = mCgiSessions.begin(); it != mCgiSessions.end(); ++it)
	{
...
void Server::closeClient(size_t i)
{
	...
	// yoksa zombie process + sizan fd. abortCgi bu client'a ait session
	// yoksa sessizce doner; pipe pollfd girislerini fd DEGERINE gore siler
	// (removePipeFromPoll), bu yuzden asagida client'in kendi girisini de
	// index yerine fd degeriyle siliyoruz -- abortCgi arada baska bir
	// indeksi silmis olsa bile guvenli.
	abortCgi(fd);
...
bool Server::isPollin(size_t i)
{
	...
	// CGI pipe'larindan biri hazir (stdout okunabilir ya da POLLHUP/POLLERR).
	if (isCgiPipe(fd))
...
void Server::listeningSockets()
{
	...
	// SHUTDOWN_DRAIN_TIMEOUT_SECONDS, CGI_TIMEOUT_SECONDS'tan (cgi.hpp,
	// 5s) BUYUK secilmeli -- yoksa hala kendi 5s hakkini kullanmakta olan
