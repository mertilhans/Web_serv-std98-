...
std::string	HttpResponseBuilder::buildError(int statusCode, const ServerConfig* cfg, bool keepAlive)
{
	...
	if (cfg)
	{
		...
		{
			// Config'teki error_page yolu (orn. /errors/404.html) root-relative
			// -- diger dosya erisimleriyle ayni mantikla root ile birlestiriliyor,
			// dogrudan diskteki mutlak yol olarak okunmuyor.
